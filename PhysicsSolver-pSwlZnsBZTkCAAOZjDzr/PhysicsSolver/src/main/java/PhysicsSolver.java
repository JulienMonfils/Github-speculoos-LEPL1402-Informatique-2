public class PhysicsSolver {
    ...
    public static class Slot {
        ...
        public boolean setValue(double value) {
            if (!hasValue()){
                ...
                if (areSameDoubles(this.value, value)){
                    return false;
                } else {
                    throw new RuntimeException();
                }
    ...
    public static class Multiplication extends Relation {
        ...
        public boolean update() {
            int number = 0;
            if (product_.hasValue()){number++;}
            if (factor1_.hasValue()){number++;}
            if (factor2_.hasValue()){number++;}
            if (number !=2){
                return false;
            }else{
                if (!product_.hasValue()){
                    product_.setValue(factor1_.getValue()*factor2_.getValue());
                }else if (!factor1_.hasValue()){
        ...
        public void clearValues() {
            getFactor1().clearValue();
            getFactor2().clearValue();
            getProduct().clearValue();
        }
    }
    ...
    public static class Squaring extends Relation {
        ...
        public boolean update() {
            int number = 0;
            if (number_.hasValue()){number++;}
            if (square_.hasValue()){number++;}
            if (number != 1){
                return false;
            }else{
                if (number_.hasValue()){
        ...
        @Override
        public void clearValues() {
            this.getNumber().clearValue();
            this.getSquare().clearValue();
        }
    }
